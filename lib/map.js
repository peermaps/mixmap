...
Map.prototype.pick = function (opts) {
  var self = this
  var ex = defined(opts.offsetX, opts.x)
  var ey = defined(opts.offsetY, opts.y)
  var x = Math.max(0, Math.min(self._size[0]-1, ex))
  var y = Math.max(0, Math.min(self._size[1]-1, self._size[1] - ey))
  var first = false
  if (!self._fb) {
    self._fb = self.regl.framebuffer({
      colorFormat: 'rgba',
      colorType: 'float32'
    })
